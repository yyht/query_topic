...
from nets.fengshen.modeling_deberta_v2 import DebertaV2ForMaskedLM
from nets.fengshen.modeling_albert import AlbertForMaskedLM

import copy

class UniMCDataset(Dataset):
    def __init__(self, data, yes_token, no_token, tokenizer, con, used_mask=True):
        super().__init__()

        self.tokenizer = tokenizer
        self.max_length = con.getint('para', 'max_length')
    ...
    def encode(self, item, used_mask=False):
        ...
        target[label_idx[item['label']]] = self.yes_token # mlm-logits
